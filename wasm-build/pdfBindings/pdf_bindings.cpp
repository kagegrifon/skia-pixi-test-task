...
static Uint8Array toBytes(sk_sp<SkData> data)
{
...
static std::unique_ptr<SkCodec> ToDecodeJpeg(sk_sp<SkData> data)
{
    return SkJpegDecoder::Decode(std::move(data), nullptr);
}

static bool ToEncodeJpeg(SkWStream *dst, const SkPixmap &src, int quality)
{
...
public:
    static CkDocument *MakePDFDocument(const SimplePDFMetadata &metadata)
    {
        ...
        pdfMetadata.jpegDecoder = ToDecodeJpeg;
        pdfMetadata.jpegEncoder = ToEncodeJpeg;
