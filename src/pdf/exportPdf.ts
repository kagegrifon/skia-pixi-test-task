...
import { renderContainer } from "../skia/SkiaRenderer";
...
async function initSkiaPdf(): Promise<CanvasKitPDF> {
  ...
  const init = (
    window as unknown as {
      CanvasKitInit: (opts: { locateFile: (f: string) => string }) => Promise<CanvasKitPDF>;
    }
  ).CanvasKitInit;
  CKPdf = await init({ locateFile: (f) => `/canvaskit/${f}` });
  return CKPdf;
}
...
export async function exportScenePdf(
  ...
  const doc = CK.MakePDFDocument({ title: "Scene Export" });
  const canvas = doc.beginPage(width, height);
  canvas.clear(CK.Color4f(1, 1, 1, 1));
  renderContainer(CK, canvas, pixiApp.stage);
  doc.endPage();
